...
├── contact.html        연락처 (이메일·주소·지도)
├── assets/
│   ├── style.css       전체 스타일 (디자인 토큰 포함)
│   ├── main.js         모바일 메뉴 토글
│   ├── logo.svg        로고 마크 (벡터, 색상 변경 가능)
│   ├── favicon.svg     브라우저 탭 아이콘 (3×3 축약형)
│   ├── icon-512.png    앱 아이콘 512px
│   ├── apple-touch-icon.png   iOS 홈화면 아이콘 180px
│   ├── logo-dark.png   어두운 배경용 (발표자료·포스터)
│   ├── logo-light.png  흰 배경용 (논문·문서)
│   └── pi.jpg          ← PI 사진을 여기에 넣으세요
└── README.md
```
...
**개념:** 흐린 점은 인구집단 전체, 진한 점과 이를 잇는 선은 위험이 높아지는 층입니다.
역학의 본질인 "집단에서 패턴을 찾는 일"을 형태로 옮겼습니다.

- **내비게이션용** — 5×3 격자 (`logo.svg`, 각 HTML 상단에 인라인 삽입)
- **파비콘용 축약형** — 3×3 격자 + 둥근 사각 배경 (`favicon.svg`)
  작은 크기에서 점이 뭉치지 않도록 별도로 단순화한 버전입니다.

**색 바꾸는 법:** `logo.svg`는 `currentColor`를 쓰므로 HTML의 `style="color:#5fd4bb"` 한 곳만
고치면 전체가 바뀝니다. 흰 배경에서는 `#0d4f45`(진한 초록)를 권장합니다.

**발표자료·포스터에 쓸 때:** `logo-dark.png` / `logo-light.png`를 그대로 삽입하시거나,
크게 확대해야 하면 `logo.svg`를 쓰시면 깨지지 않습니다.
